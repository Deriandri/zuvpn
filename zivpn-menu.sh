...
# --- LOGIKA BACKGROUND AUTO KILL (DETEKSI & TENDANG REAL) ---
if [[ "$1" == "--autokill" ]]; then
    PORT="5667"
    ...
    while true; do
        if [ -f "$ZIVPN_DB" ]; then
            ...
            while IFS='|' read -r U P E L; do
                ...
                # Deteksi User melanggar limit
                TOTAL_FOUND=0
                IPS_USER=$(echo "$LOG_NOW" | grep -w "$U" | grep "authenticated" | awk '{print $NF}' | cut -d: -f1 | sort -u)
                
                for ip in $IPS_USER; do
                ...
                if [ "$TOTAL_FOUND" -gt "$L" ]; then
                    echo "[$(date)] DETEKSI: User $U ($TOTAL_FOUND/$L IP). TENDANG!" >> $LOG_FILE
...
[Unit]
Description=ZIVPN Real Auto Kill Engine
